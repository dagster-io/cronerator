...
from cronerator import Cronerator


def get_timezone(tz_name: str):
    return zoneinfo.ZoneInfo(tz_name)

def create_datetime(year: int, month: int, day: int, *args, **kwargs):
...
def test_reversed_dst_transition_advances(execution_timezone, cron_string, times):
    ...
    for i in range(len(times) - 1):
        ...
        for j in range(i, len(times)):
            ...
            diff_str = next_time.timestamp() - times[j].timestamp()
            assert next_time.timestamp() == times[j].timestamp(), (
                f"Expected ({orig_timestamp_str}) to advance to {times[j]}, got {next_time} (Difference: {diff_str})"
            )
