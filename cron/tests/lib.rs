...
mod tests {
    ...
    fn test_cron_schedule_comprehensive() {
        ...
        let test_cases = [
            ...
            (
                "Europe/Berlin",
                "45 1 * * 1", // Every sunday at 1:45 AM
                vec![
                    (2023, 10, 15, 1, 45, 0),
                    (2023, 10, 22, 1, 45, 0),
                    (2023, 10, 29, 1, 45, 0),
            ...
            (
                "Europe/Berlin",
                "30 2 * * 1", // Every sunday at 2:30 AM
                vec![
                    (2023, 10, 15, 2, 30, 0),
                    (2023, 10, 22, 2, 30, 0),
                    (2023, 10, 29, 2, 30, 0), // fold=1 ignored
        ...
        for (case_index, (timezone_str, cron_expression, expected_datetimes)) in test_cases.iter().enumerate() {
            ...
            let first_expected = expected_datetimes[0];
            let starting_date = timezone
                .with_ymd_and_hms(first_expected.0, first_expected.1, first_expected.2 - 1, 23, 59, 59)
                .unwrap();
